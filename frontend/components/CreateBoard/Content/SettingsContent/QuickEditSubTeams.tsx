...
const QuickEditSubTeams = ({ team }: QuickEditSubTeamsProps) => {
  ...
  const handleChangeCountTeams = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    setValues((prev) => ({
      ...
      maxUsersCount:
        value !== "" ? Math.ceil(teamMembers.length / Number(currentValue)) : prev.maxUsersCount,
    }));
  };
  ...
  const handleMaxMembers = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    setValues((prev) => ({
      ...
      teamsCount:
        value !== "" ? Math.ceil(teamMembers.length / Number(currentValue)) : prev.teamsCount,
    }));
  };

  const handleSaveConfigs = () => {
    if (values.teamsCount === "" || values.maxUsersCount === "") return;
