    ...
    const renderAvatarWithButton = useCallback(
      (keyValue, idx, avatarColor, fallbackText, id) => (
        <AvatarButton
          ...
          type="button"
          isBoardsPage={isBoardsPage && !haveError}
          onClick={isBoardsPage && !haveError ? handleOpenDialog : undefined}
        >
          <Avatar
            key={`${keyValue}-${idx}-${Math.random()}`}
            colors={avatarColor}
      ...
      [handleOpenDialog, haveError, userId, isBoardsPage],
    );
