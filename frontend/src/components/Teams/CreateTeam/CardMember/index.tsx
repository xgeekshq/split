...
const CardMember = React.memo<CardBodyProps>(
	({ member, role, isTeamCreator, isNewbee, membersList }) => {
		const setMembersList = useSetRecoilState(membersListState);

		const handleIsNewbee = (checked: boolean) => {
			const listUsersMembers = membersList.map((user) => {
				return user.user._id === member._id ? { ...user, isNewbee: checked } : user;
			});
					...
					>
						...
						<Flex align="center" css={{ width: '23%' }} gap="8" justify="center">
							<ConfigurationSettings
								handleCheckedChange={handleIsNewbee}
								isChecked={isNewbee || false}
								text=""
								title="Newbee"
							/>
						</Flex>
