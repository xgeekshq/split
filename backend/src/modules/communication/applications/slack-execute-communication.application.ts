...
import { BoardRoles, BoardType } from 'src/modules/communication/dto/types';
...
export class SlackExecuteCommunication
  implements ExecuteCommunicationInterface
{
  ...
  private async postMessageOnEachChannel(teams: TeamDto[]): Promise<void> {
    ...
    const postMessagePromises = teams.map((i) =>
      this.chatHandler.postMessage(
        i.channelId as string,
        generalText[i.for](i.boardId),
      ),
    );
  ...
  private async inviteAllMembers(teams: TeamDto[]): Promise<TeamDto[]> {
    const invitePromises = teams
      ...
      .map((i) =>
        this.conversationsHandler.inviteUsersToChannel(
          i.channelId as string,
          i.participants,
        ),
      );
  ...
  private async createAllChannels(teams: TeamDto[]): Promise<TeamDto[]> {
    ...
    // !! what to do if some errors !!
    const [success, errors] = await this.resolvePromises(
      createChannelsPromises,
    );
  ...
  private async addSlackIdOnTeams(teams: TeamDto[]): Promise<TeamDto[]> {
    const usersIdsOnSlack =
      await this.conversationsHandler.getUsersFromChannelSlowly(
        this.configService.get(SLACK_MASTER_CHANNEL_ID) as string,
      );
    const usersProfiles = await this.usersHandler.getProfilesByIds(
      usersIdsOnSlack,
    );
    ...
    const teams: TeamDto[] = [
      {
        ...
        participants: board.isSubBoard
          ...
          : this.getAllUsersInDividedBoardsByRole(
              board,
  ...
  private getAllUsersInDividedBoardsByRole(
    board: BoardType,
    role: BoardRoles,
  ): UserDto[] {
