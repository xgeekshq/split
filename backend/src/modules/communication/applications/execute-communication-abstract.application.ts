...
import { BoardRoles, BoardType, ProfileType } from 'modules/communication/dto/types';
...
export abstract class ExecuteCommunicationAbstract {
	protected getMessageMasterChannel(teams: TeamDto[]): string {
		...
		return `<!channel> In order to proceed with the retro of this month, here are the random teams: \n\n
    ${textGeneralTeams} \n
	...
	protected getMessageChannel(team: TeamDto, baseUrl: string): string {
		const generalText = {
			member: (
				boardId: string
      ...
      `
		...
		return generalText[team.for](team.boardId);
	}

	protected fillUsersWithProfile(teams: TeamDto[], usersProfiles: ProfileType[]): TeamDto[] {
		return teams.map((i) => {
	...
	protected makeTeams(board: BoardType, slackChannelPrefix: string): TeamDto[] {
	...
	protected async resolvePromises(promises: Promise<any>[]): Promise<[any[], any[]]> {
